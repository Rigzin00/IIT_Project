...
export interface PolicyResponse { success: boolean; min_eligible_year: number; max_eligible_year: number; active_year: string; admin_emails: string; message?: string; }
...
export async function setPolicy(min_eligible_year: number, max_eligible_year: number, active_year: string, admin_emails?: string) {
  const res = await fetch(`${BASE}/api/admin/policy`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', ...getAuthHeaders() },
    body: JSON.stringify({ min_eligible_year, max_eligible_year, active_year, admin_emails }),
  });
  return safeJson(res);
}
